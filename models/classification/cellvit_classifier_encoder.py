...
class CellViTEncoderClassifier(nn.Module):
    """CellViT-based classifier using only encoder features (class token)
    
    This version only uses the global class token from the encoder for classification,
    which is more efficient and suitable for tasks that mainly rely on global features.
...
class CellViT256EncoderClassifier(CellViTEncoderClassifier):
...
class CellViTSAMEncoderClassifier(CellViTEncoderClassifier):
    ...
    def __init__(
        ...
        # Update classifier input dimensions
